...
class HighlightSaveView(APIView):
    ...
    def post(self, request):
        ...
        try:
                ...
                for item in data:
                    ...
                    if color:
                        ...
                        deleted_count, _ = Highlight.objects.filter(
                            user=request.user, table_name=table_name, row_id=row_id, column_id=column_id
                        ).delete()
                        if deleted_count > 0:
            ...
            return Response(
                {"error": "An internal error occurred during saving highlights."},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )

        if results["errors"] > 0:
            return Response(
                {"message": f'Processed with {results["errors"]} errors.', "details": results},
                status=status.HTTP_400_BAD_REQUEST,
            )
        else:
            return Response(
                {"message": "Highlights saved successfully.", "details": results}, status=status.HTTP_200_OK
            )
