        ...
        try:
            ...
            if ai_response.get("type") == "tool_calls":
                ...
                for tool_call in ai_response["content"]:
                    ...
                    if not tool_name or not tool_args_str or not tool_call_id:
                        ...
                        tool_results_for_next_call.append(
                            (tool_call_id, json.dumps({"status": "error", "message": "Incomplete tool call data from AI."}))
                        )
                        continue
                ...
                ai_final_response = await client.generate_response(
                    prompt=None, 
                    history=current_history_for_model, 
                    tool_results=tool_results_for_next_call
                )
                ...
                if ai_final_response.get("type") == "text":
                    ...
                    if tool_results_for_next_call and isinstance(tool_results_for_next_call, list) and len(tool_results_for_next_call) > 0:
                        try:
                ...
                # Если тип ответа "error" или любой другой неожиданный тип, но инструмент УЖЕ был выполнен
                    ...
                    logger.warning(f"Финальный ответ от AI не текст и не tool_calls, но инструменты были выполнены. Тип ответа: {ai_final_response.get('type')}, Контент: {ai_final_response.get('content')}")
                    ...
                    error_content = ai_final_response.get("content", "Неизвестная ошибка AI после вызова инструмента (или без него)")
                    response_data = {"error": error_content}
                    logger.error(f"Финальный ответ от AI - ошибка: {error_content}. Тип: {ai_final_response.get('type')}")
                    status_code = 500
            ...
            if "User message not provided" in str(ve) or "Invalid JSON" in str(ve): # Уже установлено в _get_request_data_async или проверке user_message
