...
ANALYZE_EXCEL_TOOL_SCHEMA = {
    "type": "object",
    "properties": {
        "data": {"type": "array", "items": {"type": "object"}, "description": "Данные из Excel файла"},
        "model_name": {"type": "string", "description": "Название модели Django для анализа"},
    },
            ...
            for col in columns:
                if pd.notna(row[col]):
                    ...
                    contact = contact_model.objects.filter(
                        **{f"{model_name.lower()}_id": instance.id}, contact_type=contact_type, value=row[col]
                    ).first()
