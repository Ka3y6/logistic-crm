...
class AsyncSafeTokenAuthentication(TokenAuthentication):
    ...
    async def authenticate(self, request):
        ...
        if not auth or auth[0].lower() != self.keyword.lower().encode():
            logger.debug(
                f"Authorization header not found or keyword '{self.keyword}' mismatch for request: {request.path}"
            )
        ...
        logger.debug(
            f"Attempting to authenticate with key: {key[:10]}... using sync_to_async(authenticate_credentials)"
        )
        try:
            ...
            logger.error(
                f"Unexpected error during sync_to_async(authenticate_credentials) for key {key[:10]}: {str(e)}",
                exc_info=True,
            )
