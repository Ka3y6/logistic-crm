    ...
    "function": {
        ...
        "parameters": {
            ...
            "properties": {
                "title": {
                    "type": "string",
                    "description": "The title of the calendar task. This is a required field."
                },
                "description": {
                    "type": "string",
...
def create_task_tool(
    ...
    try:
        ...
        if priority not in valid_priorities_in_model and priority is not None:
            logger.warning(f"Invalid priority '{priority}' provided for CalendarTask. Defaulting to 'medium'. Allowed: {valid_priorities_in_model}")
            priority = "medium"
        ...
        if deadline:
            try:
                ...
                logger.error(f"Invalid date format for deadline: '{deadline}'. Could not parse '{date_part}'. CalendarTask will be created without a deadline.")
        ...
        logger.info(f"CalendarTask '{task.title}' (ID: {task.id}) created successfully by user '{requesting_user.email}'.")
        ...
        logger.exception(f"Failed to create CalendarTask '{title}' by user '{requesting_user.email}'") # Добавлено .exception для трассировки
        return {
            "status": "error",
            "message": f"Failed to create CalendarTask: {str(e)}"
        }
