...
class ClientContact(models.Model):
    ...
    phone = models.CharField(
        ...
        validators=[
            RegexValidator(
                regex=r'^\+375\d{9}$',
                message="Номер телефона должен быть в формате: '+375000000000'."
            )
        ]
    )
    email = models.EmailField(
...
class Carrier(models.Model):
    ...
    fleet = models.TextField(
        _('Автопарк'),
        help_text='Информация об автопарке перевозчика',
        blank=True,
        null=True
    )
...
class Cargo(models.Model):
    weight = models.PositiveIntegerField(
        _('weight (kg)'),
        validators=[MinValueValidator(1, message="Вес должен быть больше 0.")]
    )
    ...
    tnved_code = models.CharField(
        ...
        max_length=20,
        blank=True,
        help_text='Код товарной номенклатуры'
    )
...
class Order(models.Model):
    ...
    cargo_weight = models.DecimalField(max_digits=10, decimal_places=2, verbose_name='Вес брутто (кг)', null=True, blank=True)
...
class TableHighlight(models.Model):
    ...
    table_name = models.CharField(
        max_length=100, 
        verbose_name=_('Имя таблицы')
    )
    row_id = models.IntegerField(
        verbose_name=_('ID строки')
    )
    column_id = models.CharField(
        max_length=100, 
        verbose_name=_('ID колонки')
    )
