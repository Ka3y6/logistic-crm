...
def generate_document(order, document_type):
    ...
    template_path = os.path.join(settings.BASE_DIR, "templates/documents", template_filename)
                        ...
                        if cell.value and isinstance(cell.value, str):
                            ...
                            if new_value_str != original_value:
                                if placeholder_pattern.fullmatch(original_value):
                                    final_value = get_value_from_context(
                                        context, placeholder_pattern.fullmatch(original_value).group(1)
                                    )
