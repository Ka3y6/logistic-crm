...
class LoginSerializer(serializers.Serializer):
    ...
    password = serializers.CharField(
        style={"input_type": "password"}, trim_whitespace=False, required=True, allow_blank=False
    )
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    carrier_id = serializers.PrimaryKeyRelatedField(
        queryset=Carrier.objects.all(), source="carrier", write_only=True, required=False, allow_null=True
    )
    ...
    def get_carrier(self, obj):
        if obj.carrier:
            return {"id": obj.carrier.id, "company_name": obj.carrier.company_name}
        return None
    ...
    def validate(self, data):
        if data.get("loading_date") and data.get("delivery_deadline"):
            if data["loading_date"] > data["delivery_deadline"]:
                raise serializers.ValidationError("Дата погрузки не может быть позже срока доставки")
    ...
    def to_representation(self, instance):
        ...
        if instance.client:
            representation["client"] = {"id": instance.client.id, "company_name": instance.client.company_name}

        # Добавляем информацию о перевозчике
        if instance.carrier:
            representation["carrier"] = {"id": instance.carrier.id, "company_name": instance.carrier.company_name}
...
class TaskSerializer(serializers.ModelSerializer):
    ...
    assignee_id = serializers.PrimaryKeyRelatedField(
        queryset=CustomUser.objects.all(), source="assignee", write_only=True
    )
...
class CarrierSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        representation["contacts"] = {"manager": [], "director": []}
        ...
        if color is not None and not re.match(r"^#[0-9a-fA-F]{6}$", color):
            if color != "":  # Допускаем пустую строку как null
                raise serializers.ValidationError(
                    {"color": _("Неверный формат цвета. Используйте #RRGGBB или оставьте пустым.")}
                )
        return data
